...
1. 在瀏覽器中開啟深耕計劃經費追蹤表
   - 連結：`file:///Users/yangjames/Desktop/台灣健康辦公室/0.深耕計劃入口網/gpt/深耕計劃經費追蹤表.html`
   - 或透過本地伺服器開啟（建議用 Python：`python -m http.server 8000`）
...
4. 貼入以下命令（將 URL 替換為前面複製的 Web App URL）：
   ```javascript
   localStorage.setItem('budgetSyncUrl', 'https://script.google.com/macros/s/你的WebAppID/exec');
   ```
